	...
	try {
		...
		if (!Object.hasOwn(req.body, 'file')) {
			throw new CustomError(
				'Invalid request. No file was uploaded.' +
					'Please upload an Excel file using multi-part upload.',
				400
			);
		}
		...
		if (validationError.length > 0) {
			throw new CustomError(
				'Validation error.' +
					'One or more fields failed validation. Please check the following errors',
				400,
				'validation_error',
				validationError
			);
	...
	try {
		...
		let insertedDocs = await learner.findOneAndUpdate(
			...
			},
			{
				...validatedLearner,
				institutionId: req.institution._id,
				continuing: validatedLearner.continuing
					? validatedLearner.continuing
					: req.url === '/continuing/excel',
				archived: false
			},
			{
				upsert: true,
