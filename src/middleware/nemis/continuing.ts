	...
	try {
		...
		for await (const grade of listGrades) {
			Object.assign(listLearner, {
				[grade]: (await req.nemis.listLearners(grade as Grades))
					.sort((a, b) => a.birthCertificateNo.localeCompare(b.birthCertificateNo))
			});
		}
		...
		for (let i = 0; i < continuingLearners.length; i++) {
			...
			let foundLearnerUpi = gradeListLearner.find(x => continuingLearners[i].birthCertificateNo === x.birthCertificateNo);
			if (foundLearnerUpi) {
		...
		let captureBiodataPromises = await Promise.allSettled(continuingLearners.map(x => !x?.upi ? req.nemis.addContinuingLearner(x) : <CaptureBiodataResponse>{
			upi: x?.upi,
		...
		let databaseUpdate = await Promise.all(continuingLearners.map((x, i) => learner.findByIdAndUpdate(x._id, { $set: captureBiodataUpdate(i) }, { returnDocument: 'after' })));
		...
		req.sendResponse.respond(withErrors.length > 0 ? withErrors : databaseUpdate, `Operation completed successfully ${withErrors.length > 0 ? 'with below errors' : ''}`);
