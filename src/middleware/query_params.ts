...
import {ExtendedRequest, Grades} from '../interfaces';
import logger from '../libs/logger';
import {QueryParams} from './interfaces';

//Extract allowed queryParams and return fields
	...
	try {
		...
		for (const key in queryKeys) {
			switch (queryKeys[key]) {
				case 'fields':
					let fields = req.query[queryKeys[key]]
						.toString()
						.split(',')
						.map(x => x.trim());
				...
				case 'form':
					...
					let gradeNo = queryGrade.match(/\d+/)[0];
					if (
