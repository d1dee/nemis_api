...
export const Z_STRING = z.string().toLowerCase().trim();
...
).refine(val => /^0([71])[0-9]{7,8}$/.test(val) || /^\+(?:[0-9] ?){6,14}[0-9]$/.test(val), {
...
export const Z_MEDICAL_CONDITION = Z_STRING.pipe(z.enum(MEDICAL_CONDITIONS));
