...
class NemisWebService {
    ...
    async getInstitution(institutionCode: string): Promise<Institution> {
        try {
            ...
            return institutionSchema.strip().parse({
                ...
                registrationNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Institution_Current_Code")?.attrs?.value,
                type: document
                ...
                //,
                registrationDate: document.querySelector("#ctl00_ContentPlaceHolder1_Registration_Date")?.attrs?.value,
                ward: document
                ...
                incorporationCertificateNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Registration_Certificate")?.attrs?.value,
                nearestPoliceStation: document.querySelector("#ctl00_ContentPlaceHolder1_Nearest_Police_Station")?.attrs?.value,
                nearestHealthFacility: document.querySelector("#ctl00_ContentPlaceHolder1_Nearest_Health_Facility")?.attrs?.value,
                nearestTown: document.querySelector("#ctl00_ContentPlaceHolder1_Nearest_Town")?.attrs?.value,
                //Institution Contacts Tab
                postalAddress: document.querySelector("#ctl00_ContentPlaceHolder1_Postal_Address")?.attrs?.value,
                telephoneNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Tel_Number")?.attrs?.value,
                mobileNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Mobile_Number1")?.attrs?.value,
                altTelephoneNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Tel_Number2")?.attrs?.value,
                altMobileNumber: document.querySelector("#ctl00_ContentPlaceHolder1_Mobile_Number2")?.attrs?.value,
                ...
                socialMediaHandles: document.querySelector("#ctl00_ContentPlaceHolder1_Social_Media")?.attrs?.value?.toLowerCase()
            });
        } catch (err) {
            throw err;
        }
    ...
    async listLearners(gradeOrForm: Grades): Promise<ListLearner[]> {
        try {
            let listLearnersHtml = (await this.changeResultsPerPage("/Learner/Listlearners.aspx", gradeOrForm))?.data;
            //convert table to json
            const listLearnerTable = htmlParser(listLearnersHtml).querySelector("#ctl00_ContentPlaceHolder1_grdLearners");
    ...
    async admitJoiningLearner(learner: AdmitJoiningLearner): Promise<boolean> {
        try {
            const studentIndexDocument = htmlParser((await this.axiosInstance.get("/Learner/Studindex.aspx"))?.data);
            ...
            if (!canAdmit) throw new CustomError("Admitting learners is currently disabled on the Nemis website.", 400);
            ...
            let postHtml = await this.axiosInstance({
                ...
                data: qs.stringify({
                    ...
                    ctl00$ContentPlaceHolder1$ScriptManager1: "ctl00$ContentPlaceHolder1$UpdatePanel1|ctl00$ContentPlaceHolder1$BtnAdmit",
            ...
            if (postHtml.request?.path === "/Learner/Studindexchk.aspx") {
                ...
                let postAdmHtml = await this.axiosInstance({
                    ...
                    data: {
                        ...this.#stateObject,
                        ctl00$ContentPlaceHolder1$BtnAdmit: "Admit Student",
                        ctl00$ContentPlaceHolder1$txtBCert: learner.father?.tel || learner.mother?.tel || learner.guardian?.tel,
            ...
            await writeFileSync(process.cwd() + "/debug/html/posting_admit_" + learner.indexNo + ".html", postHtml?.data);
    ...
    async listAdmittedJoiningLearners(): Promise<ListAdmittedLearner[]> {
        try {
            let admittedLearnerHtml = (await this.changeResultsPerPage("/Admission/Listlearnersrep.aspx"))?.data;
            if (!admittedLearnerHtml) {
                throw new CustomError("Failed to get hmtl table of admitted learner", 500);
            }
            
            let admittedLearnerTable = htmlParser(admittedLearnerHtml)?.querySelector("#ctl00_ContentPlaceHolder1_grdLearners")?.outerHTML;
    ...
    async captureJoiningBiodata(learner: CompleteLearner, listLearner: ListAdmittedLearner): Promise<CaptureBiodataResponse> {
        try {
    ...
    async admitDefferedLearner(defferedLearner: NemisLearnerFromDb) {
        try {
            let differedTableHtml = htmlParser.parse(await this.axiosInstance.get("/Learner/differedadmissions.aspx"))?.outerHTML;
            if (!defferedLearner)
                throw {
    ...
    async requestJoiningLearner(indexNo: string, requestingLearner: RequestingJoiningLearner & AdmitApiCall) {
        try {
            const studentIndexDocument = htmlParser((await this.axiosInstance.get("/Learner/Studindex.aspx"))?.data);
            let canAdmit = studentIndexDocument.querySelector("#txtCanAdmt")?.attrs?.value !== "0";
            let canRequest = studentIndexDocument.querySelector("#txtCanReq")?.attrs?.value !== "0";
            ...
            let postHtml = (
                await this.axiosInstance({
                    ...
                    data: qs.stringify({
                        ...
                        ctl00$ContentPlaceHolder1$txtSName: requestingLearner?.schoolAdmitted?.originalString,
                        ctl00$ContentPlaceHolder1$txtSName2: requestingLearner?.schoolAdmitted?.originalString,
                        ctl00$ContentPlaceHolder1$txtSchool: requestingLearner?.schoolSelected?.code,
            ...
            if (!/^.+pageRedirect.+Learner.+fStudindexreq/gi.test(postHtml)) {
                ...
                if (viewstateAtob && /School Vacacies are exhausted!!/.test(viewstateAtob))
                    throw {
                        message: "The School Vacancies are exhausted!!. Request for Extra" + " Slots from Director Secondary!!"
                    };
                else
                    throw {
            ...
            if (htmlParser(postHtml).querySelector("#ctl00_ContentPlaceHolder1_ErrorMessage")?.innerText === "Request Successfully Saved!!") {
                return true;
            }
            let message = htmlParser(postHtml).querySelector("#ctl00_ContentPlaceHolder1_ErrorMessage")?.innerText;
            if (message) throw { message: "Requesting learner_router failed with error: " + message };
            else await writeFileSync(process.cwd() + "/debug/html/posting_request_" + requestingLearner.indexNo + ".html", postHtml);
    ...
    async transferIn(learner: CompleteDatabaseLearner): Promise<boolean> {
        try {
            ...
            return decodeResponse.includes("The Transfer Request Saved. Learner Awaits Being Released From Current School Admitted");
        } catch (err) {
            throw err;
        }
    }
            ...
            return <(RequestedJoiningLearner & { deleteCallback: string })[]>tableToJson
                ...
                ?.map(x => {
                    ...
                    return {
                        ...
                        approved: {
                            on: <string>x["Approved On"] === "&nbsp;" ? undefined : x["Approved" + " On"],
                            by: <string>x["Approved By"] === "&nbsp;" ? undefined : x["Approved By"]
                        },
                        status: <string>x["Status"] === "&nbsp;" ? undefined : x["Status"],
                        deleteCallback: <string>x["Approved On"] === "&nbsp;" ? x[13]?.match(/ctl.*?Del/g)[0] : undefined
                    };
                })
                ?.filter(x => x);
        } catch (err) {
    ...
    async getApprovedJoiningLearners(): Promise<ApprovedLearner[]> {
        try {
            ...
            let requestedJoiningLearnerTable = htmlParser(getAllHtml).querySelector("#ctl00_ContentPlaceHolder1_grdLearners")?.outerHTML;
            if (!requestedJoiningLearnerTable) return [];
            return tableToJson
                .convert(requestedJoiningLearnerTable)
                ?.flat()
    ...
    async getRequestedContinuingLearners() {
        try {
            ...
            return <RequestingLearner[]>tableToJson
                ...
                .flat()
                .map(element => {
                    if (element["Gender"] && element["KCPE Year"] && element["Index"] && element["Birth Certificate"] && element["Grade"])
                        return {
    ...
    async requestContinuingLearners(
        ...
        try {
            ...
            let requestingLearnerHtml = (
                await this.axiosInstance({
                    ...
                    data: qs.stringify({
                        ...
                        ctl00$ContentPlaceHolder1$txtRemark: requestingLearner.remarks || "Failed" + " admission ",
                        ctl00$ContentPlaceHolder1$txtSurname: names?.surname,
                        ctl00$ContentPlaceHolder1$txtYear: requestingLearner.kcpeYear
                    })
                })
            )?.data;
            let requestedLearnersTable = htmlParser(requestingLearnerHtml).querySelector("#ctl00_ContentPlaceHolder1_grdLearners")?.outerHTML || " "; // set an empty string if querySelector returns undefined
            let requestedLearner = tableToJson
                ...
                .filter(x => x.adm === requestingLearner.adm && x.birthCertificateNo === requestingLearner.birthCertificateNo);
    ...
    async getPendingContinuingLearners(): Promise<RequestingLearner[]> {
        try {
            ...
            let pendingLearnerTable = htmlParser(pendingLearners).querySelector("#ctl00_ContentPlaceHolder1_grdLearners")?.outerHTML || " "; // Set empty string if querySelector doesn't return a value
            return tableToJson
    ...
    async captureContinuingLearners(
        ...
        try {
            ...
            if (pendingLearnerResponse?.request?.path !== "/Learner/alearner.aspx" || !pendingLearnerResponse?.data) {
    ...
    async captureBioData(learner: CompleteLearner): Promise<CaptureBiodataResponse> {
        try {
            ...
            let postData = qs.stringify({
                ...
                __VIEWSTATEENCRYPTED: "",
                __VIEWSTATEGENERATOR: this.#stateObject?.__VIEWSTATEGENERATOR,
                ctl00$ContentPlaceHolder1$DOB$ctl00: `${learner.dob.UTCTimestamp?.getMonth() + 1}/${learner.dob.UTCTimestamp.getDate()}/${learner.dob.UTCTimestamp.getFullYear()}`,
                ctl00$ContentPlaceHolder1$Nationality: nationality,
                ctl00$ContentPlaceHolder1$ScriptManager1: "ctl00$ContentPlaceHolder1$UpdatePanel1|ctl00$ContentPlaceHolder1$ddlcounty",
            ...
            let ignoreErrorPrompt = htmlParser(aLearnerHtml?.data).querySelector("#ctl00_ContentPlaceHolder1_MyDiag")?.innerText;
            
            if (ignoreErrorPrompt && /do you want to ignore this error/.test(ignoreErrorPrompt?.toLowerCase())) {
                Object.assign(formDataObject, {
                    __EVENTARGUMENT: "",
                    __EVENTTARGET: "",
                    __EVENTVALIDATION: this.#stateObject?.__EVENTVALIDATION,
            ...
            if (!message?.innerText) {
                let newUpi = aLearnerDocument.querySelector("#ctl00_ContentPlaceHolder1_instmessage")?.innerText;
            ...
            writeFileSync(process.cwd() + "/debug/html/unknown_error_" + learner.indexNo + ".html", aLearnerHtml);
            throw {
                alertMessage: message.outerHTML,
                message: message.innerText?.replace(/(^.)\n(Failure!)/, "")
            };
    ...
    async submitToNhif(grade: Grades, learnersWithoutNhif: ListLearner[]) {
        try {
            ...
            const postNhif = async (learnerWithoutNhif: ListLearner) => {
                ...
                if (!/.+pageRedirect\|\|%2fLearner%2fAlearner.aspx/.test(submitNhifHtml)) return Promise.reject("Failed to get redirect");
                ...
                if (!successMessage) return Promise.reject("Failed to get nhif number since successMessageElement is" + " empty");
                
                if (successMessage.startsWith("The remote server returned an error:")) return Promise.reject(successMessage);
                ...
                if (!parsedReturnObject.nhifNo)
                    throw {
                        message: "Failed to get nhif number since successMessage doesn't contain a" + " number",
            ...
            const selectedLearnerHtml = (
                await this.axiosInstance.post(
                    ...
                    qs.stringify({
                        ...
                        ...this.#stateObject,
                        ctl00$ContentPlaceHolder1$ScriptManager1: "ctl00$ContentPlaceHolder1$UpdatePanel1|ctl00$ContentPlaceHolder1$SelectRecs",
                        ctl00$ContentPlaceHolder1$SelectCat: "2",
                        ctl00$ContentPlaceHolder1$SelectRecs: this.recordsPerPage
                    })
                )
            )?.data;
            let selectedLearnerHtmlTable = htmlParser(selectedLearnerHtml).querySelector("#ctl00_ContentPlaceHolder1_grdLearners")?.outerHTML;
        ...
        try {
            ...
            let addLearnerBC = await this.axiosInstance.post(
                ...
                qs.stringify({
                    ...
                    __ASYNCPOST: "true",
                    __EVENTTARGET: "ctl00$ContentPlaceHolder1$SelectRecs",
                    ctl00$ContentPlaceHolder1$ScriptManager1: "ctl00$ContentPlaceHolder1$UpdatePanel1|ctl00$ContentPlaceHolder1$SelectCat",
                    ctl00$ContentPlaceHolder1$SelectBC: "1",
            ...
            throw new CustomError("Server failed to sed a redirect to biodata capture page", 400, "failed_redirect");
        } catch (err) {
            throw err;
        }
    }
        ...
        try {
            ...
            if (numberOfPages.length === 0) {
                ...
                if (url === "/Learner/Listlearners.aspx") {
                    ...
                    if (selected && selected.length === 1 && selected[0].toLowerCase().trim() === gradeOrForm) {
            ...
            throw new CustomError(err.message, err.code, err);
        }
    }
            ...
            response => {
                try {
                    ...
                    if (
                        ...
                        return Promise.reject(new CustomError("Invalid cookie. Got redirected to login page.", 401));
                    }
                    
                    if (response.data) this.#setViewState(response?.data); // Set view state
                    if (response.headers["set-cookie"]) this.#cookie = response.headers["set-cookie"]?.toString();
            ...
            err => {
                ...
                if (!err?.response) {
                    switch (err.code) {
                        case "ETIMEDOUT":
                            ...
                            break;
                        case "ECONNRESET":
                            err.message = "Connection has reset while trying to reach " + err?.config?.baseURL + err?.config?.url;
                            err.type = "connection_reset";
                            break;
                        case "ECONNABORTED":
                            err.message = "Connection was aborted while trying to reach " + err?.config?.baseURL + err?.config?.url;
                            err.type = "connection_aborted";
                            break;
                        case "ENOTFOUND":
                            err.message = `The requested address ${err?.config?.baseURL + err?.config?.url} was not found.`;
                ...
                switch (err.response?.status) {
                    ...
                    case 403:
                        ...
                        break;
                    case 504:
                        err.message = "Gateway timed out while try to reach " + err.config?.baseURL + err.config?.url;
